...
這裡的 finite 意思是數值既不是 NaN（不是數字的特殊值），也不是正負 infinity。先把這個條件講清楚很重要：一旦 loss、Q-value、target 或 gradient 已經不是有限數字，後面的 return 曲線就沒有解釋價值。
...
這個順序的重點在分支：finite check 失敗時，先定位 NaN 或 infinity 的來源；固定 batch 無法下降時，先查 forward、action gather（依 action index 挑出對應 Q 值）、target shape 和 optimizer；兩者都通過後，才有資格把注意力移到 Agent 的行為。
...
這個測試使用和 DQN 相同的資料邊界：輸入是 `(B, 4, 84, 84)` 的 normalized Tensor（PyTorch 用來做多維計算的陣列；這裡把像素值轉成模型計算用的浮點數），輸出是 `(B, 4)`，每一列代表四個 Breakout actions 的 Q-values。每一筆資料再用 action index 選出真正要比較的那一格：
...
下圖的 x 軸是 environment step，y 軸是每局完成時累積的 raw episode return。這次 run 共完成 48 局，return 介於 `0` 和 `5`，平均 `1.40`；第一局為 `2`，最後一局為 `1`。
...
要注意的是，曲線中間偶爾出現 `4` 或 `5`，不等於已經建立穩定策略；最後沒有比第一局更高，也不等於模型一定完全沒學到。這張圖最多只能說明在這個短 run 和這個 seed 下，還看不到可靠的 return 改善證據。

### Loss：finite 是必要條件，不是成功證明

這次有 `2,251` 個 finite loss samples，平均約 `0.00299`，最大值約 `0.0463`。曲線大多落在低值附近，但偶爾會出現尖峰。
...
Q-value 是模型對「在這個 state 做某個 action 有多值得」的估計；target 則是這次更新用來比較的參考值。圖中的所有 action Q mean、max、min，以及 target mean、max 都來自真實 optimizer update；這樣未被選中的 action 如果數值爆掉，也不會被 selected action 的摘要遮住。
...
這次 Q mean 大致從接近 `0` 逐步升到約 `0.26`，target max 的最高觀測值約 `1.39`，而所有值都保持 finite。這比較像是「需要繼續觀察尺度」的正常診斷訊號，不足以宣稱 Q-value 合理或不合理。真正的 warning 是相對於 reward 和 target 的尺度快速失控，或 non-finite checks 失敗；不是某個跨所有環境都適用的固定數字。
...
本次 gradient norm 最大約 `0.1952`，沒有 NaN 或 infinity；曲線仍有波動，且會在後段出現幾個較高的尖峰。這些尖峰不能單獨判定為 bug，但如果它們持續擴大、同時伴隨 loss 和 Q-value 爆升，就應該先查 reward、target mask、normalization 和 optimizer，而不是直接換演算法。
